...
后端使用 Chat Completions 兼容协议调用模型接口，并以多模态 `image_url` 格式提交图片。一次平面图识别只调用一次 `READ_MODEL`：同一请求提交转正原图、增强线稿、轮廓候选联系表和笔迹隔离图，同时返回轮廓选择、四周尺寸、门窗、高度及点位证据；尺寸绑定和几何组装随后由本地代码完成，不再逐块或逐文字追加视觉调用。
