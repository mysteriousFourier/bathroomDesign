...
Status: W1D2 baseline. Maps every Week 1 locked item from the 最小实现计划书 line 222 to an evidence row.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `EV-001` | Data dictionary | contract_document | All measurement fields; HeightParams; coordinate system; entity IDs | Review CONTRACT-BASELINE.md completeness check; validate all 4 schemas compile with `npm run validate` exit 0 | `CONTRACT-BASELINE.md`, `schemas/*.schema.json` | Data | W1D1 | confirmed | — | Business names and product-bearing IDs: `pending_business_confirmation` |
...
| `EV-005` | Sample baseline | synthetic_fixture | measurement.schema.json: boundary, walls, openings, drainagePoints, pipeEnclosures, heights | Synthetic fixture set covers rectangle, near-rectangle, single-open, multi-open, full-featured geometries; each fixture validates against measurement schema | `evidence/samples/synthetic-fixtures.md` with 5 fixture specs | Data | W1D2 | confirmed | — | Real cases: `pending_business_confirmation`; see gap resolution plan |
| `EV-006` | Threshold baseline | contract_document | measurement.schema.json coordinate tolerance, angular tolerance; rule severity levels | Every threshold row is numeric-and-sourced (from CONTRACT-BASELINE.md frozen values) or explicitly `pending_business_confirmation` | `evidence/registry/threshold-registry.json` | Data | W1D2 | confirmed | — | 24 threshold rows: 4 frozen from W1D1 contract, 20 `pending_business_confirmation` |
...
| Status | Count |
|---|---|
| confirmed | 6 (EV-001 through EV-006) |
| pending_implementation | 5 (EV-007 through EV-011) |
| pending_business_confirmation | 0 rows have this as primary status, but thresholds and real samples inside confirmed rows carry this marker |
