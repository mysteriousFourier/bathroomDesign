...
5. 读取净高和整屋吊顶。只按图中原文和单位换算，不提供示例答案。
...
2. 每个 dimension_chains.segments_mm 必须处在同一条水平或垂直尺寸线上；不能把垂直读数混进水平底链。overall_mm 必须是图中明确总尺寸，不能把不同方向数字相加。
3. 放大逐字检查 D1/W1/W2 的 CG、CK、CH。不要按常见门高猜数，但三位/四位数字必须完整抄录。
4. 门所在墙按门洞和开启圆弧位置判断。图例符号不得计入 points。
5. 净高与整屋吊顶分开；只按图中原文和单位换算。
