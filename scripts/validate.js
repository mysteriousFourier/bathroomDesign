...
function validateW1D5Artifacts(evidenceRegistry) {
  ...
  const rollupEvidence = (rollup.rows || []).find(row => row.evidenceId === 'EV-011');
  if (rollupEvidence?.status !== 'unverified') {
    errors.push(`week1 rollup EV-011 status must remain unverified, got ${rollupEvidence?.status}`);
  }
...
function main() {
  ...
  const week1Rollup = loadJSON(W1D5_ROLLUP_PATH);
