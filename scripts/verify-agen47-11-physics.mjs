...
await page.screenshot({path:'evidence/agen47-11-physics/01-wall-device-constrained.png',fullPage:true})
const wetHandle=page.locator('.dry-wet-zone-handle').nth(4)
const wetBox=await wetHandle.boundingBox()
if(!wetBox)throw new Error('wet-zone edge handle is not rendered')
await page.screenshot({path:'evidence/agen47-11-physics/02-wet-zone-before-resize.png',fullPage:true})
await page.mouse.move(wetBox.x+wetBox.width/2,wetBox.y+wetBox.height/2)
await page.mouse.down()
await page.mouse.move(wetBox.x-180,wetBox.y+wetBox.height/2,{steps:12})
await page.screenshot({path:'evidence/agen47-11-physics/03-wet-zone-live-constrained.png',fullPage:true})
await page.mouse.up()
await page.waitForTimeout(250)
await page.getByRole('button',{name:'保存',exact:true}).click()
await page.waitForTimeout(150)
await page.screenshot({path:'evidence/agen47-11-physics/04-wet-zone-committed.png',fullPage:true})
