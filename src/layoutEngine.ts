...
function graphProduct(demand: DemandProfile, category: string, quality: number, style?: string) {
  ...
  const modelBackedStyled = category === '马桶'
    ...
    : styled
  const candidates = modelBackedStyled.length ? modelBackedStyled : modelBacked
...
function retainFixtureAcrossLayouts(fixture: FixtureSpec) {
  ...
  // measured fixtures are movable evidence and must be replaced by the solved
  // product entities. The toilet drain itself is retained, while a placeholder
  // or measured toilet body is regenerated around that anchor.
...
function makeSolution(spec: RoomSpec, demand: DemandProfile, budget: BudgetTier, preference?: LayoutPreference): LayoutSolution {
  ...
  const wetPlacement=solveWetZone(spec,`${demand}-${budget}-shower`,`${budgetLabels[budget]}淋浴区`,showerSize,instruction('wet_zone'),fixedObstacles,measuredShowerDrain,wetTrace)
  ...
  fixtures.push(productFixture(`${demand}-${budget}-heater`, 'other', heaterProduct, b.minX + heaterDimensions.width_mm / 2 + 280, b.minZ + heaterDimensions.depth_mm / 2 + 20, { width_mm: 720, depth_mm: 180, height_mm: 430 }, 0, Math.max(0, Math.min(1201, (spec.height_mm ?? 2200) - heaterDimensions.height_mm - 25)), true))
  if (demand === 'laundry') {
  ...
  const verticalOverflow = fixtures.filter((f) => (f.elevation_mm ?? 0) + Math.max(1, f.height_mm) > ceilingHeight - 25)
  ...
  const checks: LayoutCheck[] = [
    ...
    check('G01-VERTICAL', verticalOverflow.length === 0, 'error', '几何', verticalOverflow.length ? `设备穿越吊顶安全界面：${verticalOverflow.map((f) => f.label).join('、')}` : '全部设备低于吊顶安全界面 25mm'),
...
function makeLevelSolution(spec:RoomSpec,level:LayoutLevelDecision,preference?:LayoutPreference):LayoutSolution{
  ...
  for(const product of products){
    ...
    if(product.category==='热水器'){elevation=Math.max(0,Math.min(1201,(spec.height_mm??2200)-dims.height_mm-25))}
...
export function applyLayoutSolution(spec: RoomSpec, solution: LayoutSolution): RoomSpec {
  ...
  const next = { ...spec, wall_finish_gap_mm: Math.max(35, spec.wall_finish_gap_mm ?? 0), fixtures: [...retainedFixtures, ...solution.fixtures.map((fixture) => ({ ...fixture, layout_generated: true }))], dry_wet_zones: retainedZones.length ? retainedZones : [solvedZone] }
